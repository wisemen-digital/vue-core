...
import { ContainsDate } from '#src/typeorm/contains-date.js'
import { Succeeds } from '#src/typeorm/succeeds.js'
import { Precedes } from '#src/typeorm/precedes.js'
import { IsPrecededBy } from '#src/typeorm/is-preceded-by.js'
import { IsSucceededBy } from '#src/typeorm/is-succeeded-by.js'
import { StartsAfter } from '#src/typeorm/starts-after.js'
...
describe('DateTimeRangeColumn', () => {
  ...
  describe('ContainsDate', () => {
    ...
    it('(-infinity, now] does not contain now + 1ms', async () => {
      ...
      const test = await dataSource.manager.findOneBy(DateTimeRangeTest, {
        id: 13,
        range: ContainsDate(now.add(1, 'ms').toDate())
      })
  ...
  describe('Succeeds', () => {
    it('finds a range that succeeds the given period', async () => {
      ...
      const periodB = new DateTimeRange(now.add(1, 'ms'), now.add(5, 'days'))
