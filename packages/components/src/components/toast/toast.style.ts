...
export const useToastStyle = tv({
  slots: {
    actionButton: 'h-5',
    actionButtonText: 'font-medium text-toast-foreground',
    actionContainer: 'mt-4',
    actionLoader: 'size-5 text-toast-foreground',
    closeButton: 'flex size-5 items-center justify-center rounded-full border border-solid border-neutral-100 bg-toast p-1',
    closeContainer: 'absolute right-0 top-0 -translate-y-1/3 translate-x-1/3',
    closeIcon: 'text-toast-foreground',
    container: 'pointer-events-auto relative z-above-everything w-[22rem] rounded-popover border border-solid border-neutral-100 bg-toast p-4 shadow-toast-shadow',
