...
const listOfComponents = components
...
function generateDocsForComponents() {
  listOfComponents.forEach((componentPath) => {
    const componentName = componentPath.split('/').pop() ?? ''
    const componentSrcFolderName = parse(componentName).dir.match(/src\/components\/([^\/]+)/)?.[1]
    const componentDocsFolderName = componentPath.split('/').pop()

    console.log({
      componentName,
      componentSrcFolderName,
      componentDocsFolderName,
    })
  })
